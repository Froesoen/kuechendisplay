...
| `display/kueche/cmd/display/power` | `on`, `off` oder `toggle` | Schaltet das Display per `wlopm` (physisch verarbeitet durch den separaten Prozess `display_power_control.py`, der denselben Topic direkt abonniert). |
...
| `display/kueche/status/state` (retain) | JSON `{"display_power":"on/off", "display_mode":"app/slideshow", "active_app":"...", "active_user":"...", "wall_mode": true/false}` | Gesamtzustand des Displays, aktualisiert bei jeder relevanten Aenderung. |
...
`cmd/button/map` umkonfigurieren lassen:
...
*Quelle: Code-Stand vom 19.09.2026 (`supervisor/kuechendisplay_supervisor.py`,
`config.py`, `button_mapper.py`, `notification_overlay.py`,
`display_power_control.py`, `slideshow/slideshow_service.py`,
`docs/diashow.md`).*
